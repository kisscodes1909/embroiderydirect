...
class THWEPO_Price {
	...
	public function define_hooks(){
		...
		if(THWEPO_Utils::is_rightpress_dynamic_pricing_plugin_active()){
			...
			if(apply_filters('thwepo_add_price_after_rightpress_discount', false)){
				// Calculate Extra price after rightpress calculation
				add_action('rightpress_product_price_changes_price_to_set',array($this, 'calculate_extraprice_after_rightpress'), $hp_bf_total, 4);

				// Add extra price if no any discout applied
				add_action('rightpress_product_price_cart_no_changes_to_prices', array($this, 'calculate_cart_extra_costs'));

				// Show cart item price value
				add_filter('rightpress_product_price_cart_item_product_display_price', array($this, 'show_cart_item_price_after_rightpress'), 10, 5);
	...
	********************************************/
	...
	public function calculate_extraprice_after_rightpress($price, $cart_item_key, $price_changes, $cart_items){

		$changed_price =  isset($price_changes['price']) ? $price_changes['price'] : $price;
				...
				if($request_data) {
					try{
						...
						if($result['code'] === 'E000'){
							...
							$extra_price = $price_data['price_extra'];

							$price = $changed_price + $extra_price;
						}
	...
	public function show_cart_item_price_after_rightpress($formatted_price, $price_breakdown_entry, $full_price, $product, $cart_item){

		$is_cart 		= isset($cart_item);
		$extra_price 	= 0;
		$current_price 	= isset($price_breakdown_entry['price']) && $price_breakdown_entry['price'] ? $price_breakdown_entry['price'] : 0;
		...
		$full_price = $full_price + $extra_price;
		$final_price = $current_price + $extra_price;
		$formated_final_price = wc_price($final_price);

		if ($is_cart && RightPress_Product_Price_Display::display_price_was_discounted($current_price, $full_price) && apply_filters('rightpress_product_price_display_full_price', false, $full_price, $final_price, $cart_item)) {
            $formatted_price = '<del>' . wc_price($full_price) . '</del> <ins>' . $formated_final_price . '</ins>';
        }
	...
	/**
	 * Get calculated extra price data.
	 *
	 * @param $args array containing field data and product details.
	 * @return array of calculated price details.
	 */
	public function get_calculated_extra_price_response($args){

		/* 
		Eg: format price info array used in the defaults argument.

		$price_info = array(
			'field_name1' => $field_info,
			'field_name2' => $field_info,
		);
		...
		*/
		...
		$price_data = $this->calculate_extra_price($args);
		$extra_price_data = $this->prepare_extra_price_response_data($price_data);

		return $extra_price_data;
	}
	...
	private function prepare_extra_price_request_data_cart($cart_item){
		...
		if(is_array($extra_options)){
			...
			$quantity = isset($cart_item['quantity']) ? $cart_item['quantity'] : false;
	...
	********************************************/
	...
	public function calculate_extra_price_ajax_handler(){
		check_ajax_referer('thwepo-extra-cost', 'security');
		...
		$response = apply_filters('thwepo_ajax_extra_option_response_data', $response, $request_data);
	...
	private function prepare_extra_price_request_data_ajax(){
		$data = false;
		$data_json = isset($_POST['price_info']) ? wc_clean(wp_unslash($_POST['price_info'])) : '';

		if($data_json) {
			$data = json_decode($data_json, true);
		}

		return apply_filters('thwepo_ajax_extra_option_posted_data', $data);
	}
	/************************************
	********* AJAX Handler - END ********
	*************************************/
	...
	***********************************************/
	private function calculate_extra_price($args, $context='product'){
		...
		foreach($price_info_list as $fname => $price_info){
			...
			if($is_flat_fee){
				...
				if($fprice > 0 && $excl_base_price){
					$make_product_price_zero = true;
				}
	...
	private function prepare_extra_price_response_data($args=array(), $context=''){
		...
		if( $product && ($resp_code === 'E000' || $resp_code === 'E001') ){
			...
			if($price_extra === 0){
				...
				$price_final = apply_filters('thwepo_product_page_display_price', $price_final, $price_original, $price_extra, $product);
			...
			$display_price_final = apply_filters('thwepo_product_price_html', $display_price_final, $product_id, $price_extra, $price_final);
			...
			$price_data = apply_filters('thwepo_extra_price_response_data', $price_data, $product_id);
	...
	private function is_valid_eval_statement($code){
	...
	***********************************************/
	...
	private function is_price_field_type_multi_option($type, $data){
		if($type && ($type === 'multiselect' || $type === 'checkboxgroup')){
			return true;
		}else if($data && $type && ($type === 'colorpalette' || $type === 'imagegroup')){
	...
	private function get_price_array_of_grouped_product(){
		$price_array = array();
		
		if($args['is_grouped_product']){
			$grouped_child_ids = $args['grouped_child_ids'];

			if($grouped_child_ids){
				$grouped_child_ids_array =  explode(',', $grouped_child_ids);
				foreach ($grouped_child_ids_array as $key => $child_id) {
					$child_product = wc_get_product($child_id);
					$price_array[$child_id] = $child_product->get_price('');
				}
			}
	...
	*****************************************/
	...
	public function render_price_table(){
		...
		if($show){
			...
			if($is_div_model === 'yes'){
				?>
				<div class="thwepo-price-table" style="display: none">
					<div class="table-item extra-price">
						<div class="label"><?php echo stripslashes(THWEPO_i18n::__t($label_extra)) ?></div>
						<div class="value"><?php echo $price_extra; ?></div>
					</div>
					<div class="table-item product-price">
						<div class="label"><?php echo stripslashes(THWEPO_i18n::__t($label_product)) ?></div>
						<div class="value"><?php echo $price; ?></div>
					</div>
					<div class="table-item total-price">
						<div class="label"><?php echo stripslashes(THWEPO_i18n::__t($label_total)) ?></div>
						<div class="value" data-price="<?php echo $product->get_price(); ?>"><?php echo $price; ?></div>
					</div>
				</div>
				<?php
			}else{
	...
	public function get_display_price_item_meta($args, $product_info, $plain = false){
		...
		if(!$is_flat_fee){
			...
			$price = apply_filters('thwepo_cart_page_item_price', $price, $args, $product_info);
