...
class THWEPO_Utils {
	...
	public static function check_specific_field_type_in_settings($field_type='inputtext'){
		$fields = self::get_custom_fields_full('true');
		$is_present = false;
	...
	public static function get_single_file_display_name( $upload_info, $downloadable ){
		...
		if($dname && $downloadable && $url){
			$show_image = apply_filters('thwepo_display_uploaded_image', false);

			if($show_image && self::is_valid_image_url($url)) {
				$dname  = '<img class="thwepo-display-img" src="'. $url .'" alt="'. $dname .'" width="30" height="30"/>';
			}else{
	...
	public static function is_valid_image_url($src_file_name) {
		$supported_image = array( 'gif','jpg','jpeg','png');
        $supported_image = apply_filters('thwepo_add_supported_image_type', $supported_image);
	...
	public static function is_aelia_currencyswitcher_plugin_active(){
		$active = is_plugin_active('woocommerce-aelia-currencyswitcher/woocommerce-aelia-currencyswitcher.php');
		return apply_filters('thwepo_aelia_currencyswitcher_plugin_enabled', $active);
	}

	public static function is_woocs_currencyswitcher_plugin_active(){
		$active = is_plugin_active('woocommerce-currency-switcher/index.php');
		return apply_filters('thwepo_woocs_currencyswitcher_plugin_enabled', $active);
	}

	public static function is_free_version_plugin_active(){
		$active = is_plugin_active('woo-extra-product-options/woo-extra-product-options.php');
		return apply_filters('thwepo_free_version_plugin_enabled', $active);
	}
