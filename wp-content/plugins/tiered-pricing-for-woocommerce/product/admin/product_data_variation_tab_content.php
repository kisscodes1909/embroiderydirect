...
<div id='wtp_product_variation_discount_meta' class='panel woocommerce_options_panel'>
	...
	<div class='wtp_options_group'>
		<?php
		$wtp_product_tier_setting = get_post_meta($variation->ID, 'wtp_product_tier_setting_' . $variation->ID, true );

		// echo '<pre> Vartion_' . $variation->ID;
		...
		if ( !empty( $wtp_product_tier_setting ) ) {
			...
			foreach ( $wtp_product_tier_setting['min_qty'] as $key => $value ) {
				...
				?>
				<div class="wtp-discount-group wtp-bulk-group bulk_range_group_<?php esc_attr_e($variation->ID); ?>_<?php esc_attr_e($i); ?>" data-index="<?php esc_attr_e($i); ?>">
					<div class="range_setter_inner">
						<div class="bulk-row-main">
							<div class="bulk-row-start wtp-input-filed-hight bulk-row-inner">
								<div class="bulk-min">
									<input type="number" name="wtp_product_tier_setting_<?php esc_attr_e($variation->ID); ?>[min_qty][]" class="bulk_discount_min wtp_value_selector wtp_next_value" placeholder="min" min="0" step="any" value="<?php esc_attr_e($min_qty); ?>">
									<span class="wtp_desc_text"><?php echo esc_html__('Minimum Quantity', 'wtp'); ?></span>
								</div>
								<div class="bulk-max">
									<input type="number" name="wtp_product_tier_setting_<?php esc_attr_e($variation->ID); ?>[max_qty][]" class="bulk_discount_max wtp_value_selector wtp_auto_add_value" placeholder="max" min="0" step="any" value="<?php esc_attr_e($max_qty); ?>">
								...
								<div class="bulk_gen_disc_type wtp-select-filed-hight">
									<select name="wtp_product_tier_setting_<?php esc_attr_e($variation->ID); ?>[discount_type][]" class="bulk-discount-type bulk_discount_select">                                        
								...
								<div class="bulk_amount">
									<input type="number" name="wtp_product_tier_setting_<?php esc_attr_e($variation->ID); ?>[discount_value][]" class="bulk_discount_value bulk_value_selector wtp_value_selector" placeholder="Discount" min="0" step="any" value="<?php esc_attr_e($discount_value); ?>">
									<span class="wtp_desc_text"><?php echo esc_html__('Discount Value', 'wtp'); ?></span>
								</div>
								<div class="bulk_gen_disc_type wtp-select-filed-hight">
									<select name="wtp_product_tier_setting_<?php esc_attr_e($variation->ID); ?>[disabled][]" class="bulk-discount-type bulk_discount_select">                                        
								...
								<div class="wtp-btn-remove">
									<span class="dashicons dashicons-no-alt wtp_variation_discount_remove" data-rmdiv="bulk_range_group_<?php esc_attr_e($variation->ID); ?>_<?php esc_attr_e($i); ?>"></span>
								</div>
							</div>
						</div>
					</div>
	...
	<div class="add-condition-and-filters wtp-discount-add-row">
		<button type="button" class="button add_variation_discount_elements" data-variation-id="<?php esc_attr_e($variation->ID); ?>" data-current-index="0" ><?php echo esc_html__('Add Rule', 'wtp'); ?></button>
	</div>
</div>
